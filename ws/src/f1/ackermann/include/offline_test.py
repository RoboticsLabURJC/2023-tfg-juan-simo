#!/usr/bin/env python
import os
import numpy as np
import matplotlib.pyplot as plt
import csv
import argparse
import cv2
import torch
from torchvision import transforms
from Reference.pilotnet import PilotNet
from datetime import datetime
from data_loader import *

TEST_PATH = "/home/juan/ros2_tfg_ws/src/f1/dl_car_control/csvs/ACK/Nurburing_ACK"
MODEL_PATH = "/home/juan/ros2_tfg_ws/src/f1/dl_car_control/models/ACK/Mono_2.pth"


# Limit velocitys
MAX_ANGULAR = 0.5
MAX_LINEAR = 20
MIN_LINEAR = 8


DEBUG = False

C_MODEL_PATH = "/home/juan/ros2_tfg_ws/src/f1/dl_car_control/models/ACK/Curves2.pth"
L_MODEL_PATH = "/home/juan/ros2_tfg_ws/src/f1/dl_car_control/models/ACK/Lines2.pth"

def denormalize(normalized_value, min, max):
    return (normalized_value * (max - min)) + min


def filter_red(image):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    lower_red = np.array([0, 70, 50])
    upper_red = np.array([10, 255, 255])
    mask1 = cv2.inRange(hsv, lower_red, upper_red)
    
    lower_red = np.array([170, 70, 50])
    upper_red = np.array([180, 255, 255])
    mask2 = cv2.inRange(hsv, lower_red, upper_red)
    
    mask = mask1 | mask2
    result = cv2.bitwise_and(image, image, mask=mask)
    
    if DEBUG:
        cv2.imshow('Filtered Red', result)
        cv2.waitKey(0)
        
    return result

def is_straight_line(contour):
    epsilon = 0.01 * cv2.arcLength(contour, True)
    approx = cv2.approxPolyDP(contour, epsilon, True)
    return len(approx) == 2  # Approximation of a straight line

def classify_line_shape(image):
    filtered_image = filter_red(image)
    gray = cv2.cvtColor(filtered_image, cv2.COLOR_BGR2GRAY)
    
    if DEBUG:
        cv2.imshow('Gray Image', gray)
        cv2.waitKey(0)
    
    edges = cv2.Canny(gray, 50, 150, apertureSize=3)
    
    if DEBUG:
        cv2.imshow('Edges', edges)
        cv2.waitKey(0)
    
    contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    if DEBUG:
        debug_img = image.copy()
        cv2.drawContours(debug_img, contours, -1, (0, 255, 0), 3)
        cv2.imshow('Contours', debug_img)
        cv2.waitKey(0)

    # Sort contours by length and keep only the two longest
    contours = sorted(contours, key=lambda x: cv2.arcLength(x, True), reverse=True)[:2]

    for contour in contours:
        if cv2.contourArea(contour) > 100:  # Ignore small contours
            if is_straight_line(contour):
                return 'LINE'
    return 'CURVE'


def main():

    # Device Selection (CPU/GPU)

    if torch.cuda.is_available():
        device = torch.device("cuda")
        device_type = "cuda"
    else:
        device = torch.device("cpu")
        device_type = "cpu"


    data_w_array= []
    net_w_array= []
    data_v_array= []
    net_v_array= []
    n_array = []
    count = 1
    

    # image_shape = (66, 200, 3)
    image_shape = [200, 66, 3]
    num_labels = 2
    input_size =[66, 200]
    
    # Curves network
    c_model = PilotNet([200, 66, 3], 2).to(device)
    c_model.load_state_dict(torch.load(C_MODEL_PATH))
        
    c_model.to(device)
        
    # Lines network
    l_model = PilotNet([200, 66, 3], 2).to(device)
    l_model.load_state_dict(torch.load(L_MODEL_PATH))
        
    l_model.to(device)

    classify = "Undetermined"

    pilotModel = PilotNet(image_shape, num_labels).to(device)
    pilotModel.load_state_dict(torch.load(MODEL_PATH, map_location=device))
    pilotModel.eval()

    preprocess = transforms.Compose([
        # convert the frame to a CHW torch tensor for training
        transforms.ToTensor()
    ]) 
    
    images = []
    labels = []

    all_images, all_data = load_data(TEST_PATH)
    images = get_images(all_images, 'cropped', images)
    labels = parse_csv(all_data, labels)

    first_line = True
    total_time = 0
    min = 20000
    max = -1
    total_mult = 0
    total_loss_v = 0
    total_loss_w = 0
    norm = False
    net = "mono"
    
    for i in tqdm(range(len(images))):
        
        start_time = datetime.now()
        
        image = images[i]
        label = labels[i]
        
        # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        # height, width
        height = image.shape[0]
        width = image.shape[1]
        
        # crop image
        # cropped_image = image[240:480, 0:640]

        resized_image = cv2.resize(image, (int(input_size[1]), int(input_size[0])))
        
        shape = classify_line_shape(resized_image)
        

        # Display cropped image
        # cv2.imshow("image", resized_image)       
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()

        input_tensor = preprocess(resized_image).to(device)

        # print(type(input_tensor))
        # The model can handle multiple images simultaneously so we need to add an
        # empty dimension for the batch.
        # [3, 200, 66] -> [1, 3, 200, 66]
        input_batch = input_tensor.unsqueeze(0)
        # Inference (min 20hz max 200hz)

        if net == "mono":
            output = pilotModel(input_batch)
        else:
            # Image inference
            if shape == 'LINE':
                output = l_model(input_batch)
                classify = "Line"
            else:
                output = c_model(input_batch)
                classify = "Curve"

        

        if device_type == "cpu":
            v = output[0].detach().numpy()[0]
            w = output[0].detach().numpy()[1]

        else:         
            v = output.data.cpu().numpy()[0][0]
            w = output.data.cpu().numpy()[0][1]


        if norm:
            v = denormalize(v, MIN_LINEAR, MAX_LINEAR)
            w = denormalize(w, -MAX_ANGULAR, MAX_ANGULAR)

        net_v_array.append(v)
        net_w_array.append(w)

        total_loss_v += (float(label[0]) - v) ** 2
        total_loss_w += (float(label[1]) - w) ** 2



        data_v_array.append(float(label[0]))
        data_w_array.append(float(label[1]))
        n_array.append(count)
        
        finish_time = datetime.now()
        dt = finish_time - start_time
        ms = (dt.days * 24 * 60 * 60 + dt.seconds) * 1000 + dt.microseconds / 1000.0
        total_time = total_time + ms
        if ms < min:
            min = ms
        if ms > max:
            max = ms
        #if not float(line[2]) == 0:
            #total_mult = total_mult + (output[0].detach().numpy()[1]/float(line[2]))
        count = count + 1
        

    
    # print("Tiempo medio:"+str(total_time/count))
    print("MSE v:"+str(total_loss_v/count))
    print("MSE w:"+str(total_loss_w/count))

    # print("Tiempo min:"+str(min))
    # print("Tiempo max:"+str(max))
    #print("Mult_w: "+ str(total_mult/count))

    
    plt.subplot(1, 2, 1)
    plt.plot(n_array, data_v_array, label = "controller", color='b')
    plt.plot(n_array, net_v_array, label = "net", color='tab:orange')
    plt.title("Lineal speed") 
    plt.legend()
    
    plt.subplot(1, 2, 2)
    plt.plot(n_array, data_w_array, label = "controller", color='b')
    plt.plot(n_array, net_w_array, label = "net", color='tab:orange')
    plt.title("Angular speed") 
    plt.legend()

    plt.show()
    
    print("FIN")



# Execute!
if __name__ == "__main__":
    main()