...
class carController(Node):
    ...
    def __init__(self):
        ...
        self.carpublisher_ = self.create_publisher(Twist, '/car_vel', 10)
    ...
    def timer_callback(self):
        if np.any(self.filteredImage):
            ...
            if red_farest[0] != 0 or red_farest[1] != 0:
                ...
                if not self.dizy: 
                    self.dizy_n = self.random_number_and_comparison(1, 100, DIZY_PERCENTAGE)
                    if self.dizy_n:
                        self.dizy = True
                    
                if (self.dizy and self.dizy_t > 0):
                    print("Dizy")
                ...
                if not self.dizy:
                    self.carpublisher_.publish(vel_msg)
